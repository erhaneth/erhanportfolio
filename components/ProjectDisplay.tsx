...
const ProjectDisplay: React.FC<ProjectDisplayProps> = ({ project, onClose }) => {
  return (
    <div className="glass-terminal border border-[#00FF41] matrix-border-glow h-full flex flex-col mono relative overflow-hidden">
      ...
      <div className="bg-[#00FF41] px-4 py-2 flex justify-between items-center">
        <span className="text-[10px] font-bold text-[#0d0208] tracking-widest uppercase"> DECIPHERED_DATA_PACKET: {project.id} </span>
        <button 
          onClick={onClose}
      ...
      <div className="p-6 space-y-8 overflow-y-auto flex-1 custom-scrollbar">
        ...
        <div className="border-l-4 border-[#00FF41] pl-4">
          <h2 className="text-2xl font-bold text-[#00FF41] tracking-tighter matrix-text-glow">{project.title.toUpperCase()}</h2>
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[10px] text-[#008F11] font-bold uppercase tracking-widest">ASSIGNMENT:</span>
            <span className="text-[10px] text-[#00FF41] bg-[#003B00] px-2 py-0.5">{project.role.toUpperCase()}</span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          <section>
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="text-xs font-bold text-[#00FF41] uppercase tracking-widest">Intelligence_briefing</h3>
            </div>
            <p className="text-sm text-[#00FF41]/90 leading-relaxed font-light">{project.description}</p>
          </section>

          <section>
            <h3 className="text-[10px] font-bold text-[#008F11] mb-2 uppercase tracking-widest">Logic_Gate_Stack</h3>
            <div className="flex flex-wrap gap-2">
              {project.technologies.map(tech => (
                <span key={tech} className="px-2 py-1 border border-[#003B00] text-[#00FF41] text-[10px] font-bold hover:bg-[#00FF41] hover:text-[#0d0208] transition-all cursor-default">
                  {tech.toUpperCase()}
                </span>
              ))}
            </div>
          ...
          {project.impact && (
            <section className="bg-[#003B00]/10 border border-[#00FF41]/20 p-4 relative">
              <div className="absolute -top-2 left-4 px-2 bg-[#020202] text-[10px] font-bold text-[#008F11]">FIELD_IMPACT</div>
              <p className="text-xs text-[#00FF41] italic font-medium leading-relaxed">"{project.impact}"</p>
            </section>
          )}
          ...
          {project.codeSnippet && (
            <section>
              <div className="flex justify-between items-center mb-2">
                <h3 className="text-[10px] font-bold text-[#008F11] uppercase tracking-widest">Kernel_Source_Partial</h3>
      ...
      <div className="p-6 bg-[#020202] border-t border-[#003B00] flex gap-4">
        {project.githubUrl && (
          <a href={project.githubUrl} target="_blank" className="flex-1 text-center py-3 border border-[#00FF41] text-[#00FF41] text-xs font-bold hover:bg-[#00FF41] hover:text-[#0d0208] transition-all shadow-[0_0_10px_rgba(0,255,65,0.2)]">
            DOWNLOAD_SRC
          </a>
        )}
